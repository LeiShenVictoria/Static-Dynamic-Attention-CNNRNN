...
if __name__ == "__main__":
	...
	parser.add_argument('--ground', help="ground truth text file, one example per line")
	parser.add_argument('--pred', help="predicted text file, one example per line")
	parser.add_argument('--emb', help="embeddings bin file", default="/users3/yfwang/rg_data/w2v/GoogleNews/GoogleNews-vectors-negative300.bin")
	args = parser.parse_args()
	
	print ("loading embeddings file...")
	w2v = Word2Vec.load_word2vec_format(args.emb, binary=True)
	#w2v = KeyedVectors.load_word2vec_format(args.emb, binary=True)
	
	r = average(args.ground, args.pred, w2v)
	print("Embedding Average Score: %f +/- %f ( %f )" %(r[0], r[1], r[2]))

	r = greedy_match(args.ground, args.pred, w2v)
	print("Greedy Matching Score: %f +/- %f ( %f )" %(r[0], r[1], r[2]))

	r = extrema_score(args.ground, args.pred, w2v)
